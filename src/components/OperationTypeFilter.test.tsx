...
describe('OperationTypeFilter', () => {
  ...
  it('renders correctly with operation types and selected types', () => {
    ...
    // Check if unknown types checkbox is rendered and unchecked by default
    const unknownTypesCheckbox = screen.getByLabelText('Includi Tipi Sconosciuti') as HTMLInputElement;
    expect(unknownTypesCheckbox).toBeInTheDocument();
    expect(unknownTypesCheckbox.checked).toBe(false);
  });
  ...
  it('toggles unknown types and calls onIncludeUnknownTypesChange', () => {
    // First test: includeUnknownTypes=false -> true
    const onIncludeUnknownTypesChangeMock = vi.fn();
    render(
      <OperationTypeFilter 
        operationTypes={mockOperationTypes} 
        selectedTypes={mockSelectedTypes}
        ...
        onIncludeUnknownTypesChange={onIncludeUnknownTypesChangeMock}
      />
    );

    // Toggle unknown types checkbox
    const unknownTypesCheckbox = screen.getByLabelText('Includi Tipi Sconosciuti');
    fireEvent.click(unknownTypesCheckbox);

    // Check if onIncludeUnknownTypesChange was called with true
    expect(onIncludeUnknownTypesChangeMock).toHaveBeenCalledTimes(1);
    expect(onIncludeUnknownTypesChangeMock).toHaveBeenCalledWith(true);
  });

  it('toggles unknown types from true to false', () => {
    // Second test: includeUnknownTypes=true -> false
    const onIncludeUnknownTypesChangeMock = vi.fn();
    render(
      <OperationTypeFilter 
        operationTypes={mockOperationTypes} 
    ...
    // Check if unknown types checkbox is checked
    const unknownTypesCheckbox = screen.getByLabelText('Includi Tipi Sconosciuti') as HTMLInputElement;
    expect(unknownTypesCheckbox.checked).toBe(true);

    // Toggle unknown types checkbox
    fireEvent.click(unknownTypesCheckbox);

    // Check if onIncludeUnknownTypesChange was called with false
    expect(onIncludeUnknownTypesChangeMock).toHaveBeenCalledTimes(1);
    expect(onIncludeUnknownTypesChangeMock).toHaveBeenCalledWith(false);
  });
});
